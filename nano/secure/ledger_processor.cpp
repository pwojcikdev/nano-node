...
namespace
{
uint64_t dependency_topo (nano::secure::write_transaction const & txn, nano::ledger & ledger, nano::block_hash const & hash)
{
	if (hash.is_zero ())
	{
		return 0;
	}
	if (auto block = ledger.store.block.get (txn, hash))
	{
		return block->sideband ().topo_height;
	}
	// Dependencies may be pruned (e.g. receives against pruned sources)
	if (ledger.store.pruned.exists (txn, hash))
	{
		return 0;
	}
	// Defensive fallback; normal ledger processing ensures dependencies exist or are pruned
	return 0;
}

uint64_t compute_topo (nano::secure::write_transaction const & txn, nano::ledger & ledger, nano::block_hash const & previous, nano::block_hash const & source)
{
	auto const prev_topo = dependency_topo (txn, ledger, previous);
	auto const source_topo = dependency_topo (txn, ledger, source);
	return std::max (prev_topo, source_topo) + 1;
}
}
					...
					if (result == nano::block_status::progress)
					{
						...
						if (result == nano::block_status::progress)
						{
							...
							if (result == nano::block_status::progress)
							{
								...
								auto const topo = compute_topo (transaction, ledger, block_a.hashables.previous, 0);
								...
								ledger.store.blocks_topo.put (transaction, topo, hash);
								...
								if (result == nano::block_status::progress)
								{
									...
									if (result == nano::block_status::progress)
									{
										...
										if (result == nano::block_status::progress)
										{
											...
											auto const topo = compute_topo (transaction, ledger, block_a.hashables.previous, block_a.hashables.source);
					...
					if (result == nano::block_status::progress)
					{
						...
						if (result == nano::block_status::progress)
						{
							...
							if (result == nano::block_status::progress)
							{
								...
								{
									ledger.store.pending.del (transaction, key);
									auto const topo = compute_topo (transaction, ledger, 0, block_a.hashables.source);
			...
			if (result == nano::block_status::progress)
			{
				...
				if (result == nano::block_status::progress)
				{
					...
					if (result == nano::block_status::progress)
					{
						...
						auto const source = is_receive ? block_a.hashables.link.as_block_hash () : nano::block_hash{ 0 };
						auto const topo = compute_topo (transaction, ledger, block_a.hashables.previous, source);
