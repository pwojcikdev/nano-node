...
uint64_t nano::ledger_processor::compute_topology_index (nano::block const & block) const
{
	uint64_t result{ 0 };

	for (auto const & dependency : topology_dependencies (block))
	{
		...
		{
			result = std::max (result, dependency_block->sideband ().topology + 1);
		}
		else
		{
			release_assert (ledger.store.pruned.exists (transaction, dependency), "topology dependency does not exist", dependency.to_string ());
		}
	}
...
void nano::ledger_processor::put_block (nano::block & block)
{
	auto hash = block.hash ();
	auto sideband = block.sideband ();
	sideband.topology = compute_topology_index (block);
	block.sideband_set (sideband);

	ledger.store.block.put (transaction, hash, block);
	ledger.store.topology.put (transaction, sideband.topology, hash);
}
