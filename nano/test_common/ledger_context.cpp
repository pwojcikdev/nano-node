...
nano::test::ledger_context::ledger_context (std::deque<nano::raw_block> && blocks) :
	...
	pool_m{ nano::dev::network_params.network, 1 }
{
	...
	for (auto const & i : blocks_m)
	{
		...
		debug_assert (process_result.code == nano::block_status::progress, to_string (process_result.code));
	}
}
...
std::deque<nano::raw_block> const & nano::test::ledger_context::blocks () const
{
	return blocks_m;
}
...
auto nano::test::ledger_diamond (unsigned height) -> ledger_context
{
	...
	using account_block_pair = std::pair<nano::keypair, nano::raw_block>;
	std::deque<account_block_pair> previous;
	previous.push_back ({ nano::dev::genesis_key, nano::dev::genesis.raw () });
	...
	for (unsigned level = 0; level < height; ++level)
	{
		...
		{
			auto [key, root] = std::move (previous.front ());
			previous.pop_front ();
			...
			blocks.push_back (std::move (open1));
			blocks.push_back (std::move (open2));
		}
		previous.clear ();
	...
	while (previous.size () > 1)
	{
		...
		while (!previous.empty ())
		{
			auto [key1, root1] = std::move (previous.front ());
			previous.pop_front ();
			auto [key2, root2] = std::move (previous.front ());
			previous.pop_front ();
			...
			blocks.push_back (std::move (receive2));
		}
		previous.clear ();
		previous.swap (current);
	}
...
auto nano::test::ledger_single_chain (unsigned height) -> nano::test::ledger_context
{
	...
	nano::block_builder builder;
	nano::block_hash previous_hash = nano::dev::genesis.hash ();
	for (unsigned i = 0; i < height / 4; ++i)
	{
		auto send1 = builder.state ()
					 ...
					 .link (nano::dev::genesis_key.pub)
					 .sign (nano::dev::genesis_key.prv, nano::dev::genesis_key.pub)
					 .work (*pool.generate (previous_hash))
					 .build ();
