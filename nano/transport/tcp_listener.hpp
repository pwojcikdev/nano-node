...
{
struct tcp_listener_params
{
	uint16_t default_port{ 0 };
	bool dev_network{ false };
...
public:
	using excluded_peer_callback = std::function<bool (asio::ip::address const &)>;
	using create_server_callback = std::function<std::shared_ptr<nano::transport::tcp_server> (std::shared_ptr<nano::transport::tcp_socket> const &)>;

	tcp_listener (asio::io_context &, uint16_t port, tcp_config const &, nano::stats &, nano::logger &, tcp_listener_params const &, excluded_peer_callback, create_server_callback, std::unique_ptr<nano::transport::tcp_acceptor> = nullptr);
	~tcp_listener ();
...
private: // Dependencies
	...
	tcp_listener_params const params;
	nano::stats & stats;
	nano::logger & logger;
	excluded_peer_callback excluded_peer;
	create_server_callback create_server;
...
private:
	...
	std::unique_ptr<nano::transport::tcp_acceptor> acceptor;
