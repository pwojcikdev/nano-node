...
nano::transport::tcp_channels::tcp_channels (asio::io_context & io_ctx_a, nano::network_constants const & network_constants_a, nano::stats & stats_a, nano::logger & logger_a, tcp_channels_params const & params_a, should_pass_callback should_pass_a, not_a_peer_callback not_a_peer_a, excluded_peer_callback excluded_peer_a, random_fill_callback random_fill_a, start_tcp_callback start_tcp_a, channel_connected_callback channel_connected_a) :
	io_ctx{ io_ctx_a },
	network_constants{ network_constants_a },
	stats{ stats_a },
	logger{ logger_a },
	params{ params_a },
	should_pass{ std::move (should_pass_a) },
	not_a_peer{ std::move (not_a_peer_a) },
	excluded_peer{ std::move (excluded_peer_a) },
	random_fill_cb{ std::move (random_fill_a) },
	start_tcp_impl{ std::move (start_tcp_a) },
	channel_connected{ std::move (channel_connected_a) }
{
}
...
bool nano::transport::tcp_channels::check (const nano::tcp_endpoint & endpoint, const nano::account & node_id) const
{
	...
	if (not_a_peer && not_a_peer (nano::transport::map_tcp_to_endpoint (endpoint), params.allow_local_peers))
	{
		stats.inc (nano::stat::type::tcp_channels_rejected, nano::stat::detail::not_a_peer);
		logger.debug (nano::log::type::tcp_channels, "Rejected invalid endpoint channel: {}", endpoint);
	...
	if (has_duplicate)
	{
		stats.inc (nano::stat::type::tcp_channels_rejected, nano::stat::detail::channel_duplicate);
		logger.debug (nano::log::type::tcp_channels, "Rejected duplicate channel: {} ({})", endpoint, node_id.to_node_id ());
...
std::shared_ptr<nano::transport::tcp_channel> nano::transport::tcp_channels::create (const std::shared_ptr<nano::transport::tcp_socket> & socket, const std::shared_ptr<nano::transport::tcp_server> & server, const nano::account & node_id)
{
	...
	{
		stats.inc (nano::stat::type::tcp_channels, nano::stat::detail::channel_rejected);
		logger.debug (nano::log::type::tcp_channels, "Rejected channel: {} ({})", endpoint, node_id.to_node_id ());
		// Rejection reason should be logged earlier

		return nullptr;
	}

	stats.inc (nano::stat::type::tcp_channels, nano::stat::detail::channel_accepted);
	logger.debug (nano::log::type::tcp_channels, "Accepted channel: {} ({}) ({})",
	...
	auto should_pass_l = [this] (std::size_t size, nano::transport::traffic_type type) {
		return should_pass ? should_pass (size, type) : true;
	};
	auto channel = std::make_shared<nano::transport::tcp_channel> (io_ctx, stats, params.protocol_version, std::move (should_pass_l), socket);
...
nano::tcp_endpoint nano::transport::tcp_channels::bootstrap_peer ()
{
	...
	{
		if (i->channel->get_network_version () >= params.protocol_version_min)
		{
...
{
	if (params.disable_max_peers_per_ip)
	{
		return false;
	}
	bool result{ false };
	...
	nano::unique_lock<nano::mutex> lock{ mutex };
	result = channels.get<ip_address_tag> ().count (address) >= params.max_peers_per_ip;
	if (!result)
	{
		result = attempts.get<ip_address_tag> ().count (address) >= params.max_peers_per_ip;
	}
	if (result)
	{
		stats.inc (nano::stat::type::tcp, nano::stat::detail::max_per_ip, nano::stat::dir::out);
	}
	return result;
}
...
{
	if (params.disable_max_peers_per_subnetwork)
	{
		return false;
	}
	bool result{ false };
	...
	nano::unique_lock<nano::mutex> lock{ mutex };
	result = channels.get<subnetwork_tag> ().count (subnet) >= params.max_peers_per_subnetwork;
	if (!result)
	{
		result = attempts.get<subnetwork_tag> ().count (subnet) >= params.max_peers_per_subnetwork;
	}
	if (result)
	{
		stats.inc (nano::stat::type::tcp, nano::stat::detail::max_per_subnetwork, nano::stat::dir::out);
	}
	return result;
}
...
bool nano::transport::tcp_channels::track_reachout (nano::endpoint const & endpoint_a)
{
	...
	if (excluded_peer && excluded_peer (tcp_endpoint))
	{
		return false;
	}
	if (params.disable_tcp_realtime)
	{
		return false;
	}
...
void nano::transport::tcp_channels::purge (std::chrono::steady_clock::time_point cutoff_deadline)
{
	...
	auto should_close = [this, cutoff_deadline] (auto const & channel) {
		...
		{
			stats.inc (nano::stat::type::tcp_channels_purge, nano::stat::detail::idle);
			logger.debug (nano::log::type::tcp_channels, "Closing idle channel: {} (idle for {}s)",
			channel->to_string (),
		...
		if (channel->get_network_version () < params.protocol_version_min)
		{
			stats.inc (nano::stat::type::tcp_channels_purge, nano::stat::detail::outdated);
			logger.debug (nano::log::type::tcp_channels, "Closing channel with old protocol version: {}", channel->to_string ());
	...
	{
		stats.inc (nano::stat::type::tcp_channels, nano::stat::detail::erase_dead);
		logger.debug (nano::log::type::tcp_channels, "Removing dead channel: {}", connection.channel->to_string ());
...
{
	nano::messages::keepalive message{ network_constants };
	if (random_fill_cb)
	{
		random_fill_cb (message.peers);
	}

	nano::unique_lock<nano::mutex> lock{ mutex };

	auto const cutoff_time = std::chrono::steady_clock::now () - params.keepalive_period;
...
{
	return start_tcp_impl ? start_tcp_impl (endpoint) : false;
}
