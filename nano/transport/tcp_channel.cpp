...
nano::transport::tcp_channel::tcp_channel (asio::io_context & io_ctx_a, nano::stats & stats_a, uint8_t protocol_version, should_pass_callback should_pass_a, std::shared_ptr<nano::transport::tcp_socket> socket_a) :
	channel{ stats_a, protocol_version },
	io_ctx{ io_ctx_a },
...
bool nano::transport::tcp_channel::send_impl (nano::messages::message const & message, nano::transport::traffic_type type, nano::transport::channel::callback_t callback)
{
	...
	if (!queue.full (type))
	{
		...
		stats.inc (nano::stat::type::tcp_channel, nano::stat::detail::queued, nano::stat::dir::out);
		stats.inc (nano::stat::type::tcp_channel_queued, to_stat_detail (type), nano::stat::dir::out);
	...
	else
	{
		...
		stats.inc (nano::stat::type::tcp_channel, nano::stat::detail::drop, nano::stat::dir::out);
		stats.inc (nano::stat::type::tcp_channel_drop, to_stat_detail (type), nano::stat::dir::out);
	}
	return false;
}
...
asio::awaitable<boost::system::error_code> nano::transport::tcp_channel::send_one (traffic_type type, tcp_channel_queue::entry_t const & item)
{
	...
	while (allocated_bandwidth < size)
	{
		...
		if (should_pass (bandwidth_chunk, type)) // Allocate bandwidth in larger chunks
		{
			allocated_bandwidth += bandwidth_chunk;
		}
		else
		{
			stats.inc (nano::stat::type::tcp_channel_wait, nano::stat::detail::wait_bandwidth, nano::stat::dir::out);
			co_await nano::async::sleep_for (100ms); // TODO: Exponential backoff
		}
	}
	allocated_bandwidth -= size;

	stats.inc (nano::stat::type::tcp_channel, nano::stat::detail::send, nano::stat::dir::out);
	stats.inc (nano::stat::type::tcp_channel_send, to_stat_detail (type), nano::stat::dir::out);
	...
	if (!ec)
	{
		stats.add (nano::stat::type::traffic_tcp_type, to_stat_detail (type), nano::stat::dir::out, size_written);
		set_last_packet_sent (std::chrono::steady_clock::now ());
	}
	else
	{
		stats.inc (nano::stat::type::tcp_channel_ec, nano::to_stat_detail (ec), nano::stat::dir::out);
	}
