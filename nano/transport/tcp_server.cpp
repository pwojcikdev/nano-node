	...
	while (!co_await nano::async::cancelled ())
	{
		...
		if (message)
		{
			...
			if (realtime_visitor.process)
			{
				...
				bool added = ctx.message_sink->enqueue (std::move (message), channel);
...
auto nano::transport::tcp_server::process_handshake (nano::messages::node_id_handshake const & message) -> asio::awaitable<handshake_status>
{
	...
	if (message.response)
	{
		if (ctx.handshake_handler->verify_handshake_response (*message.response, get_remote_endpoint ()))
		{
...
{
	auto query = ctx.handshake_handler->prepare_handshake_query (get_remote_endpoint ());
...
{
	auto response = ctx.handshake_handler->prepare_handshake_response (query, version);
	auto own_query = ctx.handshake_handler->prepare_handshake_query (get_remote_endpoint ());
