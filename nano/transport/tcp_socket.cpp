...
nano::transport::tcp_socket::tcp_socket (asio::io_context & io_ctx_a, nano::transport::tcp_config const & config_a, nano::stats & stats_a, nano::logger & logger_a, nano::transport::socket_endpoint endpoint_type_a) :
	io_ctx{ io_ctx_a },
	config{ config_a },
	stats{ stats_a },
	logger{ logger_a },
...
nano::transport::tcp_socket::tcp_socket (asio::io_context & io_ctx_a, nano::transport::tcp_config const & config_a, nano::stats & stats_a, nano::logger & logger_a, asio::ip::tcp::socket raw_socket_a, nano::transport::socket_endpoint endpoint_type_a) :
	io_ctx{ io_ctx_a },
	config{ config_a },
	stats{ stats_a },
	logger{ logger_a },
...
void nano::transport::tcp_socket::close_impl ()
{
	...
	if (!ec)
	{
		stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::close);
		logger.debug (nano::log::type::tcp_socket, "Closed socket: {}", remote_endpoint);
	}
	else
	{
		stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::close_error);
		logger.debug (nano::log::type::tcp_socket, "Closed socket, ungracefully: {} ({})", remote_endpoint, ec.message ());
	}
}
...
auto nano::transport::tcp_socket::ongoing_checkup () -> asio::awaitable<void>
{
	...
	try
	{
		while (!co_await nano::async::cancelled () && alive ())
		{
			...
			if (!healthy)
			{
				stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::unhealthy);
				logger.debug (nano::log::type::tcp_socket, "Unhealthy socket detected: {} (timed out: {})",
				remote_endpoint,
				timed_out.load ());
			...
			{
				std::chrono::seconds sleep_duration = config.checkup_interval;
...
bool nano::transport::tcp_socket::checkup ()
{
	...
	if (connected)
	{
		...
		{
			stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::already_closed);
			return false; // Bad
		}
		...
		std::chrono::seconds const io_threshold = config.io_timeout;
		std::chrono::seconds const silence_threshold = config.silent_timeout;
		...
		if (io_threshold.count () > 0)
		{
			...
			{
				stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::timeout);
				stats.inc (nano::stat::type::tcp_socket_timeout, nano::stat::detail::timeout_receive);
				stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_io_timeout_drop, nano::stat::dir::in);
			...
			{
				stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::timeout);
				stats.inc (nano::stat::type::tcp_socket_timeout, nano::stat::detail::timeout_send);
				stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_io_timeout_drop, nano::stat::dir::out);
		...
		if (silence_threshold.count () > 0)
		{
			...
			{
				stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::timeout);
				stats.inc (nano::stat::type::tcp_socket_timeout, nano::stat::detail::timeout_silence);
				stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_silent_connection_drop, nano::stat::dir::in);
	...
	else // Not connected yet
	{
		...
		auto const cutoff = now - config.connect_timeout;

		if (time_created < cutoff)
		{
			stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::timeout);
			stats.inc (nano::stat::type::tcp_socket_timeout, nano::stat::detail::timeout_connect);
...
auto nano::transport::tcp_socket::co_connect_impl (nano::endpoint endpoint) -> asio::awaitable<std::tuple<boost::system::error_code>>
{
	...
	if (!ec)
	{
		...
		stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_connect_success);
		stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::connect_success);
		logger.debug (nano::log::type::tcp_socket, "Successfully connected to: {} from local: {}",
		remote_endpoint, local_endpoint);
	}
	else
	{
		stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_connect_error);
		stats.inc (nano::stat::type::tcp_socket, nano::stat::detail::connect_error);
		logger.debug (nano::log::type::tcp_socket, "Failed to connect to: {} ({})",
...
auto nano::transport::tcp_socket::co_read_impl (nano::shared_buffer buffer, size_t target_size) -> asio::awaitable<std::tuple<boost::system::error_code, size_t>>
{
	...
	if (!ec)
	{
		...
		stats.add (nano::stat::type::traffic_tcp, nano::stat::detail::all, nano::stat::dir::in, size_read);
	}
	else
	{
		stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_read_error);
		logger.debug (nano::log::type::tcp_socket, "Error reading from: {} ({})", remote_endpoint, ec);
...
auto nano::transport::tcp_socket::co_write_impl (nano::shared_buffer buffer, size_t target_size) -> asio::awaitable<std::tuple<boost::system::error_code, size_t>>
{
	...
	{
		last_send = timestamp;
		stats.add (nano::stat::type::traffic_tcp, nano::stat::detail::all, nano::stat::dir::out, size_written);
	}
	else
	{
		stats.inc (nano::stat::type::tcp, nano::stat::detail::tcp_write_error);
		logger.debug (nano::log::type::tcp_socket, "Error writing to: {} ({})", remote_endpoint, ec);
