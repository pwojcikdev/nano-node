...
public:
	explicit tcp_channels (transport_context &);
	~tcp_channels ();

	void start ();
	void stop ();

	std::shared_ptr<tcp_channel> create (std::shared_ptr<tcp_socket> const &, std::shared_ptr<tcp_server> const &, nano::account const & node_id, nano::node_capabilities_flags = {});
	void erase (nano::tcp_endpoint const &);
	std::size_t size () const;
	std::shared_ptr<tcp_channel> find_channel (nano::tcp_endpoint const &) const;
	void random_fill (std::array<nano::endpoint, 8> &) const;
	std::shared_ptr<tcp_channel> find_node_id (nano::account const &);
	...
	using channel_filter = std::function<bool (std::shared_ptr<channel> const &)>;
	std::deque<std::shared_ptr<channel>> list (channel_filter) const;
	std::deque<std::shared_ptr<channel>> list (uint8_t minimum_version = 0) const;
	std::unordered_set<std::shared_ptr<channel>> random_set (std::size_t max_count, uint8_t minimum_version = 0) const;
