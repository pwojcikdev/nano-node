...
{
struct tcp_channels_params
{
	bool allow_local_peers{ false };
	bool disable_max_peers_per_ip{ false };
	bool disable_max_peers_per_subnetwork{ false };
	bool disable_tcp_realtime{ false };
	std::size_t max_peers_per_ip{ 0 };
	std::size_t max_peers_per_subnetwork{ 0 };
	uint8_t protocol_version{ 0 };
	uint8_t protocol_version_min{ 0 };
	std::chrono::milliseconds keepalive_period{ 0 };
};
...
public:
	using should_pass_callback = std::function<bool (std::size_t, nano::transport::traffic_type)>;
	using not_a_peer_callback = std::function<bool (nano::endpoint const &, bool)>;
	using excluded_peer_callback = std::function<bool (nano::tcp_endpoint const &)>;
	using random_fill_callback = std::function<void (std::array<nano::endpoint, 8> &)>;
	using start_tcp_callback = std::function<bool (nano::endpoint const &)>;
	using channel_connected_callback = std::function<void (std::shared_ptr<nano::transport::tcp_channel> const &)>;

	tcp_channels (asio::io_context &, nano::network_constants const &, nano::stats &, nano::logger &, tcp_channels_params const &, should_pass_callback, not_a_peer_callback, excluded_peer_callback, random_fill_callback, start_tcp_callback, channel_connected_callback);
	~tcp_channels ();
...
private: // Dependencies
	...
	nano::stats & stats;
	nano::logger & logger;
	tcp_channels_params const params;
	should_pass_callback should_pass;
	not_a_peer_callback not_a_peer;
	excluded_peer_callback excluded_peer;
	random_fill_callback random_fill_cb;
	start_tcp_callback start_tcp_impl;
	channel_connected_callback channel_connected;
