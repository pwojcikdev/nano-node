...
nano::transport::channel::channel (nano::stats & stats_a, uint8_t protocol_version) :
	stats{ stats_a }
{
	set_network_version (protocol_version);
}
...
bool nano::transport::channel::send (nano::messages::message const & message, nano::transport::traffic_type traffic_type, callback_t callback)
{
	...
	stats.inc (sent ? nano::stat::type::message : nano::stat::type::drop, to_stat_detail (message.type ()), nano::stat::dir::out, /* aggregate all */ true);
	return sent;
}
