...
nano::transport::tcp_listener::tcp_listener (asio::io_context & io_ctx_a, uint16_t port_a, tcp_config const & config_a, nano::stats & stats_a, nano::logger & logger_a, tcp_listener_params const & params_a, excluded_peer_callback excluded_peer_a, create_server_callback create_server_a, std::unique_ptr<nano::transport::tcp_acceptor> acceptor_a) :
	io_ctx{ io_ctx_a },
	config{ config_a },
	params{ params_a },
	stats{ stats_a },
	...
	create_server{ std::move (create_server_a) },
	port{ port_a },
	strand{ io_ctx_a.get_executor () },
	acceptor{ acceptor_a ? std::move (acceptor_a) : std::make_unique<nano::transport::asio_tcp_acceptor> (strand) },
	task{ strand }
{
}
...
void nano::transport::tcp_listener::start ()
{
	...
	try
	{
		...
		logger.debug (nano::log::type::tcp_listener, "Listening for incoming connections on: {}", local);
	}
...
void nano::transport::tcp_listener::stop ()
{
	...
	acceptor->close (ec); // Interrupt pending accept operations before waiting for tasks
	if (ec)
	{
...
asio::awaitable<void> nano::transport::tcp_listener::wait_available_slots () const
{
	...
	{
		if (log_interval.elapse (params.dev_network ? 1s : 15s))
		{
...
auto nano::transport::tcp_listener::accept_one (asio::ip::tcp::socket raw_socket, connection_type type) -> accept_return
{
	...
	auto socket = std::make_shared<nano::transport::tcp_socket> (io_ctx, config, stats, logger, std::move (raw_socket), to_socket_endpoint (type));
	auto server = create_server ? create_server (socket) : nullptr;
	if (!server)
	{
		stats.inc (nano::stat::type::tcp_listener, nano::stat::detail::accept_rejected, to_stat_dir (type));
		logger.debug (nano::log::type::tcp_listener, "Rejected connection due to missing server factory: {} ({})", remote_endpoint, to_string (type));
		socket->close_async ();
		return { accept_result::rejected };
	}
...
auto nano::transport::tcp_listener::check_limits (asio::ip::address const & ip, connection_type type) const -> accept_result
{
	...
	if (excluded_peer && excluded_peer (ip)) // true => error
	{
	...
	{
		if (auto count = count_per_ip (ip); count >= params.max_peers_per_ip)
		{
	...
	if (!params.disable_max_peers_per_subnetwork && !nano::transport::is_ipv4_or_v4_mapped_address (ip))
	{
		if (auto count = count_per_subnetwork (ip); count >= params.max_peers_per_subnetwork)
		{
