	...
	benchmark_base (node_a, config_a)
{
	...
	node->ledger_notifications.blocks_processed.add ([this] (std::deque<std::pair<nano::block_status, nano::block_context>> const & batch) {
		...
		for (auto const & [status, context] : batch)
		{
			...
			{
				if (auto it = locked->find (context.block->hash ()); it != locked->end ())
				{
...
void pipeline_benchmark::run_benchmark ()
{
	...
	for (size_t iteration = 0; iteration < config.num_iterations; ++iteration)
	{
		...
		measure_confirmation_performance (blocks);

		std::cout << fmt::format ("Iteration {} complete.\n", iteration + 1);
		std::cout << fmt::format ("  Processed: {} blocks\n", processed_blocks_count.load ());
...
void pipeline_benchmark::measure_confirmation_performance (std::deque<std::shared_ptr<nano::block>> & blocks)
{
	...
	// Initialize timing entries for all blocks
	{
		...
		for (auto const & block : blocks)
		{
			...
			locked->emplace (block->hash (), timing);
			pending_locked->emplace (block->hash (), now);
		}
	}
	...
	while (true)
	{
		...
		// Done when no blocks are pending cementing
		if (pending_count == 0)
		{
			break;
		}
		...
		{
			std::cout << fmt::format ("{}/{} confirmed, {}/{} cemented, {} pending (elections: {}, cementing: {}, deferred: {}, pool: {})\n",
			current_confirmed, total_blocks,
			current_cemented, total_blocks,
			pending_count,
			node->active.size (),
	...
	std::cout << fmt::format ("Full confirmation pipeline completed in {:.2f}s ({} blocks/sec)\n",
...
void pipeline_benchmark::print_timing_statistics ()
{
	...
	{
		std::cout << fmt::format ("Average block processing time: {:.2f} ms\n",
		total_processing_time_us.load () / (confirmed_count * 1000.0));
...
void pipeline_benchmark::run (boost::program_options::variables_map const & vm, std::filesystem::path const & data_path)
{
	auto config = benchmark_config::parse (vm);

	std::cout << fmt::format ("Starting full pipeline benchmark with {} accounts, {} iterations, {} batch size\n",
	...
	node_config.priority_scheduler.max_blocks = std::numeric_limits<size_t>::max (); // Unlimited priority bucket
