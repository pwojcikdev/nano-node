...
void throughput_account_pool::set_frontier (nano::account const & account, nano::block_hash const & frontier)
{
	frontiers[account] = frontier;
}

nano::block_hash throughput_account_pool::get_frontier (nano::account const & account) const
{
...
throughput_benchmark::throughput_benchmark (std::shared_ptr<nano::node> node_a, size_t accounts, size_t iterations, size_t batch_size) :
	node (node_a), num_accounts (accounts), num_iterations (iterations), batch_size (batch_size)
{
...
void run_benchmark_block_processing (boost::program_options::variables_map const & vm, std::filesystem::path const & data_path)
{
	...
	{
		batch_size = std::stoull (vm["batch_size"].as<std::string> ());
	}

	std::cout << fmt::format ("Starting throughput benchmark with {} accounts, {} iterations, {} batch size\n",
	num_accounts, num_iterations, batch_size);
	...
	// Run benchmark
	throughput_benchmark benchmark{ node, num_accounts, num_iterations, batch_size };
