...
nano::messages::message_header::message_header (nano::network_constants const & constants, message_type type_a) :
...
nano::messages::message_header::message_header (bool & error_a, nano::stream & stream_a)
{
	if (!error_a)
	{
...
void nano::messages::message_header::serialize (nano::stream & stream_a) const
{
...
bool nano::messages::message_header::deserialize (nano::stream & stream_a)
{
	auto error (false);
	try
	{
...
nano::block_type nano::messages::message_header::block_type () const
{
	return static_cast<nano::block_type> (((extensions & block_type_mask) >> 8).to_ullong ());
}

void nano::messages::message_header::block_type_set (nano::block_type type_a)
{
	extensions &= ~block_type_mask;
	extensions |= (extensions_bitset_t{ static_cast<unsigned long long> (type_a) } << 8);
}

uint8_t nano::messages::message_header::count_get () const
{
...
void nano::messages::message_header::count_set (uint8_t count_a)
{
	debug_assert (type == message_type::confirm_ack || type == message_type::confirm_req);
	debug_assert (!flag_test (confirm_v2_flag)); // Only valid for v1
...
uint8_t nano::messages::message_header::count_v2_get () const
{
	debug_assert (type == message_type::confirm_ack || type == message_type::confirm_req);
...
void nano::messages::message_header::count_v2_set (uint8_t count)
{
	debug_assert (type == message_type::confirm_ack || type == message_type::confirm_req);
	debug_assert (flag_test (confirm_v2_flag)); // Only valid for v2
...
bool nano::messages::message_header::flag_test (uint8_t flag) const
{
...
void nano::messages::message_header::flag_set (uint8_t flag, bool enable)
{
	// Extension bits at index >= 8 are block type & count
	debug_assert (flag < 8);
...
bool nano::messages::message_header::bulk_pull_is_count_present () const
{
	auto result (false);
...
bool nano::messages::message_header::bulk_pull_ascending () const
{
	auto result (false);
	if (type == message_type::bulk_pull)
	{
...
bool nano::messages::message_header::frontier_req_is_only_confirmed_present () const
{
	auto result (false);
...
bool nano::messages::message_header::confirm_is_v2 () const
{
	debug_assert (type == message_type::confirm_ack || type == message_type::confirm_req);
	return flag_test (confirm_v2_flag);
}

void nano::messages::message_header::confirm_set_v2 (bool value)
{
	debug_assert (type == message_type::confirm_ack || type == message_type::confirm_req);
	flag_set (confirm_v2_flag, value);
}

std::size_t nano::messages::message_header::payload_length_bytes () const
{
	switch (type)
	{
...
bool nano::messages::message_header::is_valid_message_type () const
{
	switch (type)
	{
		case message_type::bulk_pull:
...
void nano::messages::message_header::operator() (nano::object_stream & obs) const
{
	obs.write ("type", type);
