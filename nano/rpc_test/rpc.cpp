...
TEST (rpc, account_balance)
{
	...
	ASSERT_TIMELY (5s, !node->active.active (send1.qualified_root ()));
...
TEST (rpc, send_epoch_2)
{
	...
	ASSERT_TIMELY (5s, nano::test::confirmed (*node, { epoch1->hash (), epoch2->hash () }));
...
TEST (rpc, history)
{
	...
	ASSERT_TRUE (send);
	auto receive (system.wallet (0)->receive_action (send->hash (), nano::dev::genesis_key.pub, node0->config.receive_minimum.number (), send->link_field ()->as_account ()));
	ASSERT_TRUE (receive);
	nano::block_builder builder;
	auto usend = builder
				 .state ()
				   ...
				   .build ();
	{
		...
		ASSERT_EQ (nano::block_status::progress, node0->ledger.process (transaction, usend).code);
		ASSERT_EQ (nano::block_status::progress, node0->ledger.process (transaction, ureceive).code);
		ASSERT_EQ (nano::block_status::progress, node0->ledger.process (transaction, uchange).code);
	}
...
TEST (rpc, account_history)
{
	...
	// Test include_linked_account
	{
		...
		ASSERT_EQ (usend.account_field ().value ().to_account (), std::get<2> (history_l[0]));
		ASSERT_EQ ("send", std::get<0> (history_l[1]));
		ASSERT_EQ (usend.hash ().to_string (), std::get<1> (history_l[1]));
		ASSERT_EQ (ureceive.account_field ().value ().to_account (), std::get<2> (history_l[1]));
	}
	...
	ASSERT_TRUE (send2);
	auto receive2 (system.wallet (0)->receive_action (send2->hash (), account2, node0->config.receive_minimum.number (), send2->link_field ()->as_account ()));
...
TEST (rpc, history_count)
{
	...
	ASSERT_TRUE (send);
	auto receive (system.wallet (0)->receive_action (send->hash (), nano::dev::genesis_key.pub, node->config.receive_minimum.number (), send->link_field ()->as_account ()));
	ASSERT_TRUE (receive);
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "history");
...
TEST (rpc, work_generate_block_low)
{
	...
	block.set_work (system.work_generate_limited (block.root ().as_block_hash (), threshold, nano::difficulty::from_multiplier (node->config.max_work_generate_multiplier / 10, threshold)));
	nano::block_hash hash (block.root ().as_block_hash ());
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	block.set_work (system.work_generate_limited (block.root ().as_block_hash (), 1, threshold - 1));
	nano::block_hash hash (block.root ().as_block_hash ());
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "work_generate");
...
TEST (rpc, account_representative_set_epoch_2_insufficient_work)
{
	...
	auto head = system.upgrade_genesis_epoch (*node, nano::epoch::epoch_2);
	ASSERT_TRUE (head);
...
TEST (rpc, accounts_balances_unopened_account_with_receivables)
{
	...
	ASSERT_TIMELY (5s, !node->active.active (send.qualified_root ()));
...
TEST (rpc, pending_exists)
{
	...
	ASSERT_TIMELY (5s, !node->active.active (block1->qualified_root ()));
	...
	ASSERT_TRUE (pending_exists ("1"));
	reset_confirmation_height (node->store, block1->account_field ().value ());
	ASSERT_TRUE (pending_exists ("0"));
	request.put ("include_only_confirmed", "false");
	ASSERT_TRUE (pending_exists ("1"));
}
...
TEST (rpc, wallet_receivable)
{
	...
	reset_confirmation_height (system.nodes.front ()->store, block1->account_field ().value ());
...
TEST (rpc, search_receivable_all)
{
	...
	{
		auto transaction = node->ledger.tx_begin_write ();
		ASSERT_EQ (nano::block_status::progress, node->ledger.process (transaction, block).code);
	}
	auto const rpc_ctx = add_rpc (system, node);
	boost::property_tree::ptree request;
...
TEST (rpc, account_info)
{
	...
	nano::keypair key1;
	{
		...
		auto open = builder
					...
					.build ();
		ASSERT_EQ (nano::block_status::progress, node1->process (open));
		ASSERT_TIMELY (5s, !node1->active.active (state_change.qualified_root ()));
		ASSERT_TIMELY (5s, !node1->active.active (open.qualified_root ()));
	}
...
TEST (rpc, blocks_info_receive_hash)
{
	...
	auto recv1 (system.wallet (0)->receive_action (send1->hash (), key1.pub, node->config.receive_minimum.number (), send1->link_field ()->as_account ()));
	auto recv4 (system.wallet (0)->receive_action (send4->hash (), key1.pub, node->config.receive_minimum.number (), send4->link_field ()->as_account ()));
	auto recv3 (system.wallet (0)->receive_action (send3->hash (), key1.pub, node->config.receive_minimum.number (), send3->link_field ()->as_account ()));
	auto recv2 (system.wallet (0)->receive_action (send2->hash (), key1.pub, node->config.receive_minimum.number (), send2->link_field ()->as_account ()));
...
TEST (rpc, blocks_info_subtype)
{
	...
	ASSERT_TRUE (send);
	auto receive (system.wallet (0)->receive_action (send->hash (), key.pub, nano::Knano_ratio, send->link_field ()->as_account ()));
	ASSERT_TRUE (receive);
...
TEST (rpc, block_info)
{
	...
	// Test include_linked_account - send block
	{
		...
		request.put ("include_linked_account", "true");
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (response.get<std::string> ("linked_account"), send.destination_field ().value ().to_account ());
	}
	// Test include_linked_account - receive block
	{
		...
		request.put ("include_linked_account", "true");
		auto response (wait_response (system, rpc_ctx, request));
		ASSERT_EQ (response.get<std::string> ("linked_account"), nano::dev::genesis_key.pub.to_account ());
	}
}
...
TEST (rpc, block_create_state_request_work)
{
	...
	std::optional<nano::raw_block> epoch2;
	{
...
TEST (rpc, online_reps)
{
	...
	auto receive (system.wallet (1)->receive_action (send->hash (), new_rep, node1->config.receive_minimum.number (), send->link_field ()->as_account ()));
	ASSERT_TRUE (receive);
...
TEST (rpc, block_confirmed)
{
	...
	// Open an account directly in the ledger
	{
		...
		auto open1 = builder
					 ...
					 .work (*system.work.generate (key.pub))
					 .build ();
		ASSERT_EQ (nano::block_status::progress, node->ledger.process (transaction, open1).code);
	}
