...
inline db_val::db_val (nano::topology_key const & value) :
	buffer{ std::make_shared<std::vector<uint8_t>> () }
{
	{
		nano::vectorstream stream{ *buffer };
		nano::write (stream, boost::endian::native_to_big (value.index));
...
inline db_val::operator nano::topology_key () const
{
	nano::bufferstream stream{ span_view.data (), span_view.size () };
	nano::topology_key result;
	nano::read (stream, result.index);
	boost::endian::big_to_native_inplace (result.index);
