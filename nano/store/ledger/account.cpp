...
void account::put (nano::store::write_transaction const & transaction, nano::account const & account, nano::account_info const & info)
{
...
std::optional<nano::account_info> account::get (nano::store::transaction const & transaction, nano::account const & account) const
{
...
void account::del (nano::store::write_transaction const & transaction, nano::account const & account)
{
	auto status = backend.del (transaction, tables::accounts, account);
	backend.release_assert_success (status);
}

bool account::exists (nano::store::transaction const & transaction, nano::account const & account) const
{
	return backend.exists (transaction, tables::accounts, account);
}

size_t account::count (nano::store::transaction const & transaction) const
{
	return backend.count (transaction, tables::accounts);
}

auto account::begin (nano::store::transaction const & transaction, nano::account const & account) const -> iterator
{
	return iterator{ backend.begin (transaction, tables::accounts, account) };
}

auto account::begin (nano::store::transaction const & transaction) const -> iterator
{
	return iterator{ backend.begin (transaction, tables::accounts) };
}

auto account::rbegin (nano::store::transaction const & transaction) const -> reverse_iterator
{
	return reverse_iterator{ std::prev (end (transaction)) };
}

auto account::rend (nano::store::transaction const & transaction) const -> reverse_iterator
{
	return reverse_iterator{ end (transaction) };
}

auto account::end (nano::store::transaction const & transaction) const -> iterator
{
