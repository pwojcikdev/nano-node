...
void pruned::put (store::write_transaction const & transaction, nano::block_hash const & hash)
{
	auto status = backend.put (transaction, tables::pruned, hash, nullptr);
	backend.release_assert_success (status);
}

void pruned::del (store::write_transaction const & transaction, nano::block_hash const & hash)
{
	auto status = backend.del (transaction, tables::pruned, hash);
	backend.release_assert_success (status);
}

bool pruned::exists (store::transaction const & transaction, nano::block_hash const & hash) const
{
	return backend.exists (transaction, tables::pruned, hash);
}

nano::block_hash pruned::random (store::transaction const & transaction) const
{
...
size_t pruned::count (store::transaction const & transaction) const
{
...
auto pruned::begin (store::transaction const & transaction, nano::block_hash const & hash) const -> iterator
{
	return iterator{ backend.begin (transaction, tables::pruned, hash) };
}

auto pruned::begin (store::transaction const & transaction) const -> iterator
{
	return iterator{ backend.begin (transaction, tables::pruned) };
}

auto pruned::end (store::transaction const & transaction) const -> iterator
{
	return iterator{ backend.end (transaction, tables::pruned) };
}

void pruned::for_each_par (std::function<void (store::read_transaction const &, iterator, iterator)> const & action) const
{
