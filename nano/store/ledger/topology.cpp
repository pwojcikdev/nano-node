...
{
	auto status = backend.put (txn, nano::store::table::topology, nano::topo_key{ index, hash }, nullptr);
	backend.release_assert_success (status);
}

void topology_view::del (nano::store::write_transaction const & txn, uint64_t index, nano::block_hash const & hash)
{
	auto status = backend.del (txn, nano::store::table::topology, nano::topo_key{ index, hash });
	backend.release_assert_success (status);
}

bool topology_view::exists (nano::store::transaction const & txn, uint64_t index, nano::block_hash const & hash) const
{
	return backend.exists (txn, nano::store::table::topology, nano::topo_key{ index, hash });
}
...
{
	return iterator{ backend.begin (txn, nano::store::table::topology, nano::topo_key{ index, 0 }) };
}
