...
public:
	...
	// Table operations
	bool empty (store::transaction const & tx) const; // Checks if all tables are empty
	...
	virtual bool table_exists (store::transaction const & tx, std::string const & name) const = 0;
