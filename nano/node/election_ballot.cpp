...
std::chrono::seconds nano::vote_cooldown (nano::uint128_t weight, nano::uint128_t online_stake)
{
	...
	switch (nano::calculate_rep_tier (weight, online_stake))
	{
...
auto nano::election_ballot::insert (std::shared_ptr<nano::block> const & block, nano::uint128_t cached_tally) -> insert_result
{
	...
	auto const weights = compute_weights ();
	...
	for (auto const & [held_hash, held_block] : blocks_m)
	{
		...
		auto tallied = weights.block_weights.find (held_hash);
		nano::uint128_t const held_weight = tallied != weights.block_weights.end () ? tallied->second : 0;
...
nano::tally_map nano::election_ballot::make_tally (std::unordered_map<nano::block_hash, nano::uint128_t> const & block_weights) const
{
...
auto nano::election_ballot::evaluate (nano::uint128_t quorum_threshold) -> round
{
	...
	// The winner may have no votes at all, e.g. the initial block before any vote arrived, so both lookups may miss
	result.winner = winner ();
	if (auto tallied = block_weights.find (winner_m); tallied != block_weights.end ())
	{
		result.winner_weight = tallied->second;
	}
	if (auto tallied = final_weights.find (winner_m); tallied != final_weights.end ())
	{
		result.final_weight = tallied->second;
	}
	...
	result.final_quorum = result.final_weight >= quorum_threshold;
