...
/**
 * Validated handle for wallet-key crypto. The only way to obtain one is via
 * wallet_store::unlock(), which has therefore already validated the password.
 * The cipher exposes encrypt/decrypt but never the underlying wallet key,
 * so it is impossible to encrypt or decrypt with an unvalidated key.
 */
class wallet_cipher final
{
public:
	wallet_cipher (wallet_cipher const &) = default;
	wallet_cipher (wallet_cipher &&) = default;
	wallet_cipher & operator= (wallet_cipher const &) = default;
	wallet_cipher & operator= (wallet_cipher &&) = default;
