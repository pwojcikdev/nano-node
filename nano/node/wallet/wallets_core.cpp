...
wallets_core::wallets_core (nano::wallet::wallets & facade_a, nano::wallet::wallets_backend & backend_a, nano::ledger & ledger_a, nano::node_config const & config_a, nano::network_params const & network_params_a, nano::logger & logger_a) :
	facade{ facade_a },
	backend{ backend_a },
	ledger{ ledger_a },
	config{ config_a },
...
nano::container_info wallets_core::container_info () const
{
...
wallet_data::wallet_data (nano::store::write_transaction & transaction_a, wallets_core & core_a, nano::wallet_id const & id_a) :
	id{ id_a },
	store{ core_a.kdf, transaction_a, core_a.backend, core_a.config.random_representative (), core_a.config.password_fanout, id_a.to_string () },
	handle{ std::make_shared<wallet> (core_a.facade, id_a) }
{
}

wallet_data::wallet_data (nano::store::write_transaction & transaction_a, wallets_core & core_a, nano::wallet_id const & id_a, std::string const & json) :
	id{ id_a },
	store{ core_a.kdf, transaction_a, core_a.backend, core_a.config.password_fanout, id_a.to_string (), json },
	handle{ std::make_shared<wallet> (core_a.facade, id_a) }
{
}

std::shared_ptr<wallet> wallets_core::open (nano::wallet_id const & id)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
...
std::shared_ptr<wallet> wallets_core::create (nano::wallet_id const & id)
{
...
std::shared_ptr<wallet> wallets_core::create_from_json (nano::wallet_id const & id, std::string const & json)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
	debug_assert (items.find (id) == items.end ());
...
bool wallets_core::destroy (nano::wallet_id const & id)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
...
bool wallets_core::exists (nano::account const & account)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto transaction = tx_begin_read ();
...
bool wallets_core::exists_any (nano::account const & account1, nano::account const & account2)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto transaction = tx_begin_read ();
	return std::any_of (items.begin (), items.end (), [&] (auto const & item) {
...
nano::store::write_transaction wallets_core::tx_begin_write ()
{
	return backend.tx_begin_write ();
}

nano::store::read_transaction wallets_core::tx_begin_read () const
{
	return backend.tx_begin_read ();
}

void wallets_core::clear_send_ids ()
{
	auto transaction = tx_begin_write ();
	backend.send_action_ids_clear (transaction);
}

std::vector<std::pair<nano::wallet_id, nano::account>> wallets_core::holders (nano::account const & account) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto transaction = tx_begin_read ();
...
std::unordered_map<nano::wallet_id, std::shared_ptr<wallet>> wallets_core::all_wallets ()
{
	nano::lock_guard<nano::mutex> lock{ mutex };
...
std::vector<nano::wallet_id> wallets_core::wallet_ids () const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
...
std::size_t wallets_core::wallet_count () const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	return items.size ();
}

wallet_data * wallets_core::find_wallet (nano::wallet_id const & id) const
{
	auto existing = items.find (id);
	return existing != items.end () ? existing->second.get () : nullptr;
}

bool wallets_core::enter_initial_password (nano::wallet_id const & id)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
	...
	{
		auto transaction = tx_begin_read ();
		return !enter_password_impl (*wallet_l, transaction, wallet_store::default_password);
	}
	return false;
}

bool wallets_core::enter_password (nano::wallet_id const & id, std::string const & password)
{
	auto transaction = tx_begin_write ();
...
bool wallets_core::enter_password_impl (wallet_data & wallet_l, nano::store::transaction const & transaction, std::string const & password)
{
...
bool wallets_core::rekey (nano::wallet_id const & id, std::string const & password)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
bool wallets_core::is_locked (nano::wallet_id const & id) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
bool wallets_core::lock (nano::wallet_id const & id)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
void wallets_core::set_lock_observer (nano::wallet_id const & id, std::function<void (bool, bool)> observer)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
nano::public_key wallets_core::deterministic_insert_impl (wallet_data & wallet_l, nano::store::write_transaction const & transaction, nano::wallet::wallet_cipher const & cipher)
{
...
nano::public_key wallets_core::deterministic_insert_impl (wallet_data & wallet_l, nano::store::write_transaction const & transaction, nano::wallet::wallet_cipher const & cipher, uint32_t index)
{
...
nano::result<nano::public_key> wallets_core::deterministic_insert (nano::wallet_id const & id, uint32_t index)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	...
	auto result = deterministic_insert_impl (*wallet_l, transaction, cipher.value (), index);
	lock.unlock ();
	transaction.commit ();
	return result;
}

nano::result<nano::public_key> wallets_core::deterministic_insert (nano::wallet_id const & id)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	...
	auto result = deterministic_insert_impl (*wallet_l, transaction, cipher.value ());
	lock.unlock ();
	transaction.commit ();
	return result;
}

nano::result<nano::public_key> wallets_core::insert_adhoc (nano::wallet_id const & id, nano::raw_key const & prv)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
bool wallets_core::insert_watch (nano::wallet_id const & id, nano::public_key const & pub)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
void wallets_core::remove_account (nano::wallet_id const & id, nano::account const & account)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
std::vector<nano::account> wallets_core::accounts (nano::wallet_id const & id) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
bool wallets_core::exists (nano::wallet_id const & id, nano::account const & account) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
nano::result<bool> wallets_core::move_accounts (nano::wallet_id const & target, nano::wallet_id const & source, std::vector<nano::public_key> const & accounts)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
...
key_type wallets_core::key_type (nano::wallet_id const & id, nano::account const & account) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
nano::result<nano::raw_key> wallets_core::get_seed (nano::wallet_id const & id) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
nano::result<nano::public_key> wallets_core::change_seed (nano::wallet_id const & id, nano::raw_key const & seed, uint32_t count)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
void wallets_core::deterministic_restore (nano::wallet_id const & id)
{
	auto transaction = tx_begin_write ();
	nano::unique_lock<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
std::optional<uint32_t> wallets_core::deterministic_check (nano::wallet_id const & id, uint32_t index) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
std::optional<uint32_t> wallets_core::deterministic_check_impl (wallet_data const & wallet_l, nano::store::transaction const & transaction, nano::wallet::wallet_cipher const & cipher, uint32_t index) const
{
...
std::optional<nano::public_key> wallets_core::deterministic_insert_up_to_impl (wallet_data & wallet_l, nano::store::write_transaction const & transaction, nano::wallet::wallet_cipher const & cipher, uint32_t last)
{
	...
	{
		account = deterministic_insert_impl (wallet_l, transaction, cipher);
...
nano::public_key wallets_core::change_seed_impl (wallet_data & wallet_l, nano::store::write_transaction const & transaction, nano::wallet::wallet_cipher const & cipher, nano::raw_key const & seed, uint32_t count)
{
...
uint32_t wallets_core::get_deterministic_index (nano::wallet_id const & id) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
void wallets_core::set_representative (nano::wallet_id const & id, nano::account const & rep)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
nano::account wallets_core::get_representative (nano::wallet_id const & id) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
nano::result<nano::raw_key> wallets_core::fetch_prv (nano::wallet_id const & id, nano::account const & account) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
prepared_block wallets_core::prepare_receive (nano::wallet_id const & id, nano::block_hash const & send_hash, nano::account const & representative, nano::uint128_union const & amount, nano::account const & account, uint64_t work)
{
...
prepared_block wallets_core::prepare_change (nano::wallet_id const & id, nano::account const & source, nano::account const & representative, uint64_t work)
{
	std::shared_ptr<nano::block> block;
	nano::block_details details;
	{
...
prepared_send wallets_core::prepare_send (nano::wallet_id const & id, nano::account const & source, nano::account const & account, nano::uint128_t const & amount, uint64_t work, std::optional<std::string> send_id)
{
...
void wallets_core::work_update_impl (wallet_data & wallet_l, nano::store::write_transaction const & transaction, nano::account const & account, nano::root const & root, uint64_t work)
{
...
void wallets_core::update_work (nano::wallet_id const & id, nano::account const & account, nano::root const & root, uint64_t work)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
nano::result<uint64_t> wallets_core::get_work (nano::wallet_id const & id, nano::public_key const & pub) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
void wallets_core::set_work (nano::wallet_id const & id, nano::public_key const & pub, uint64_t work)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
nano::result<wallet_scan_info> wallets_core::scan_info (nano::wallet_id const & id) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
bool wallets_core::import (nano::wallet_id const & id, std::string const & json, std::string const & password)
{
	auto transaction = tx_begin_write ();
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
...
void wallets_core::serialize_json (nano::wallet_id const & id, std::string & json) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
void wallets_core::write_backup (nano::wallet_id const & id, std::filesystem::path const & path) const
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
	if (!wallet_l)
...
nano::fan & wallets_core::password_fan (nano::wallet_id const & id)
{
	nano::lock_guard<nano::mutex> lock{ mutex };
	auto wallet_l = find_wallet (id);
