	...
	id{ wallet_id_a }
{
	...
	try
	{
		release_assert (entry_get_raw (transaction_a, version_special).key.is_zero (), "wallet_store json constructor expects empty wallet");
		...
		for (auto i (wallet_l.begin ()), n (wallet_l.end ()); i != n; ++i)
		{
			...
			entry_put_raw (transaction_a, key, wallet_value{ value, 0 });
		}
		bool missing = false;
	...
	id{ wallet_id_a }
{
	...
	try
	{
		if (entry_get_raw (transaction_a, version_special).key.is_zero ())
		{
			...
			entry_put_raw (transaction_a, wallet_store::salt_special, wallet_value{ salt_l, 0 });
			...
			entry_put_raw (transaction_a, wallet_store::wallet_key_special, wallet_value{ encrypted, 0 });
			...
			entry_put_raw (transaction_a, wallet_store::check_special, wallet_value{ check, 0 });
			nano::raw_key rep;
			rep.bytes = representative_a.bytes;
			entry_put_raw (transaction_a, wallet_store::representative_special, wallet_value{ rep, 0 });
			nano::raw_key seed;
			random_pool::generate_block (seed.bytes.data (), seed.bytes.size ());
			seed_set (transaction_a, seed);
			entry_put_raw (transaction_a, wallet_store::deterministic_index_special, wallet_value{ 0, 0 });
		}
		nano::raw_key key;
		key = entry_get_raw (transaction_a, wallet_store::wallet_key_special).key;
		wallet_key_mem.value_set (key);
...
{
	wallet_value value{ entry_get_raw (transaction_a, wallet_store::representative_special) };
...
nano::public_key wallet_store::insert_adhoc (nano::store::write_transaction const & transaction_a, nano::raw_key const & prv)
{
	...
	entry_put_raw (transaction_a, pub, wallet_value{ ciphertext, 0 });
	return pub;
}
...
bool wallet_store::insert_watch (nano::store::write_transaction const & transaction_a, nano::account const & pub_a)
{
	...
	if (!error)
	{
		entry_put_raw (transaction_a, pub_a, wallet_value{ nano::raw_key (0), 0 });
	}
	return error;
}
...
wallet_value wallet_store::entry_get_raw (nano::store::transaction const & transaction_a, nano::account const & pub_a) const
{
	return backend.get (transaction_a, handle, pub_a);
}

void wallet_store::entry_put_raw (nano::store::write_transaction const & transaction_a, nano::account const & pub_a, wallet_value const & entry_a)
{
	backend.put (transaction_a, handle, pub_a, entry_a);
}

nano::wallet::key_type wallet_store::key_type (wallet_value const & value_a) const
{
...
{
	wallet_value value{ entry_get_raw (transaction_a, wallet_store::version_special) };
...
void wallet_store::version_put (nano::store::write_transaction const & transaction_a, unsigned version_a)
{
	...
	entry_put_raw (transaction_a, wallet_store::version_special, wallet_value{ entry, 0 });
}
...
{
	wallet_value value{ entry_get_raw (transaction_a, wallet_store::check_special) };
	return value.key;
}

nano::uint256_union wallet_store::salt (nano::store::transaction const & transaction_a) const
{
	wallet_value value{ entry_get_raw (transaction_a, wallet_store::salt_special) };
	return value.key;
}
...
nano::raw_key wallet_store::seed (nano::store::transaction const & transaction) const
{
	...
	wallet_value value{ entry_get_raw (transaction, wallet_store::seed_special) };
...
void wallet_store::seed_set (nano::store::write_transaction const & transaction_a, nano::raw_key const & prv_a)
{
	...
	entry_put_raw (transaction_a, wallet_store::seed_special, wallet_value{ ciphertext, 0 });
...
nano::public_key wallet_store::deterministic_insert (nano::store::write_transaction const & transaction_a)
{
	...
	marker <<= 32;
	marker |= index;
	entry_put_raw (transaction_a, result, wallet_value{ marker, 0 });
	++index;
...
{
	wallet_value value{ entry_get_raw (transaction_a, wallet_store::deterministic_index_special) };
...
void wallet_store::deterministic_clear (nano::store::write_transaction const & transaction_a)
{
	...
	{
		switch (key_type (wallet_value{ i->second }))
		{
			case key_type::deterministic:
			{
...
bool wallet::search_receivable_impl (nano::store::transaction const & wallet_transaction_a)
{
	...
	if (!result)
	{
		...
		for (auto i (store.begin (wallet_transaction_a)), n (store.end (wallet_transaction_a)); i != n; ++i)
		{
			...
			if (!wallet_value{ i->second }.key.is_zero ())
			{
