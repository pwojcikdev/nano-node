...
void nano::election::broadcast_vote_locked (std::chrono::steady_clock::time_point now)
{
	...
	// The evaluation re-tallies a live election and may switch its winner, so the outgoing vote always references the current leader; a sealed election reports its frozen round
	auto const round = evaluate_locked ();
