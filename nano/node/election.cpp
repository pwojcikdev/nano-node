...
nano::election_actions nano::election::tick (std::chrono::steady_clock::time_point now)
{
	...
	switch (state_m)
	{
		...
		case nano::election_state::active:
			...
			actions.broadcast = pacing.due_block (ballot.winner ()->hash (), now);
...
nano::election_ballot::round nano::election::evaluate_locked ()
{
	...
	status.final_tally = round.final_weight;
...
bool nano::election::publish (std::shared_ptr<nano::block> const & block_a)
{
	...
	switch (result.outcome)
	{
		...
		case nano::election_ballot::insert_outcome::rejected:
			node.network.filter.clear (block_a); // Not backed by enough weight to take part, allow receiving it again
			return true;
	}
	debug_assert (false);
	return true;
...
void nano::election::broadcast_vote_locked (std::chrono::steady_clock::time_point now)
{
	...
	bool const is_final = confirmed_locked () || evaluate_locked ().quorum;
