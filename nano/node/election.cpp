...
nano::election_actions nano::election::tick (std::chrono::steady_clock::time_point now)
{
	...
	switch (state_m)
	{
		case nano::election_state::passive:
			...
			break;
		case nano::election_state::active:
			actions.vote = decide_vote_locked (now);
...
{
	nano::lock_guard<nano::mutex> guard{ mutex };
	return decide_vote_locked (std::chrono::steady_clock::now ());
}

std::optional<nano::election_actions::vote_broadcast> nano::election::decide_vote_locked (std::chrono::steady_clock::time_point now)
{
	debug_assert (!mutex.try_lock ());
	...
	if (confirmed_locked () || have_quorum (tally_impl ()))
	{
		...
		return nano::election_actions::vote_broadcast{
			.qualified_root = qualified_root,
			.hash = status.winner->hash (),
			.bucket = bucket,
			.type = nano::vote_type::final,
		};
	}
	else
	{
