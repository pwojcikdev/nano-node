...
void nano::election::broadcast_sent (nano::block_hash const & winner)
{
	...
	bool const initial = pacing.first_block ();
...
nano::election_actions nano::election::tick (std::chrono::steady_clock::time_point now)
{
	...
	switch (state_m)
	{
		case nano::election_state::passive:
			...
			break;
		case nano::election_state::active:
			actions.vote = decide_vote (now);
			if (pacing.due_block (status.winner->hash (), now))
			{
				actions.broadcast = snapshot_locked ();
			}
			if (pacing.due_request (behavior_m, now))
			{
				actions.request = snapshot_locked ();
			}
			break;
		case nano::election_state::confirmed:
			...
			if (pacing.due_block (status.winner->hash (), now))
			{
				actions.broadcast = snapshot_locked (); // Ensure election winner is broadcasted
			}
...
std::optional<nano::election_actions::vote_broadcast> nano::election::decide_vote ()
{
	nano::lock_guard<nano::mutex> guard{ mutex };
	return decide_vote (std::chrono::steady_clock::now ());
}

std::optional<nano::election_actions::vote_broadcast> nano::election::decide_vote (std::chrono::steady_clock::time_point now)
{
	debug_assert (!mutex.try_lock ());
	...
	if (confirmed_locked () || ballot.evaluate (quorum_delta ()).quorum)
	{
		...
		return nano::election_actions::vote_broadcast{ status.winner->hash (), nano::vote_type::final };
	}
	else
	{
		...
		return nano::election_actions::vote_broadcast{ status.winner->hash (), nano::vote_type::normal };
	}
}
