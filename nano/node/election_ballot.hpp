...
public: // Tally
	struct round final
	{
		...
		nano::uint128_t winner_weight{ 0 }; // Vote weight behind the winner
		...
		bool quorum{ false }; // Winner leads every other held block by at least the quorum threshold
...
private:
	// Vote weight and final-vote weight per voted-for block hash, including unheld hashes
	...
	nano::tally_map make_tally (std::unordered_map<nano::block_hash, nano::uint128_t> const & block_weights) const;
