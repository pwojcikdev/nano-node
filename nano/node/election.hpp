...
{
	struct vote_broadcast final
	{
		nano::block_hash hash;
		nano::vote_type type;
	};

	// Broadcast our vote for the current winner
	std::optional<vote_broadcast> vote;
	// Broadcast the current winner block
	std::optional<nano::election_snapshot> broadcast;
	// Solicit votes from representatives
	std::optional<nano::election_snapshot> request;
...
public: // State transitions
	...
	// Decide whether a vote broadcast for the current winner is due, without waiting for a tick
	std::optional<nano::election_actions::vote_broadcast> decide_vote ();
...
private:
	...
	// Tracks votes and competing blocks, computes the tally and quorum
...
private:
	...
	/**
	 * Decide whether to broadcast a vote for the current winner. Requests a final vote if reached quorum or already confirmed
	 * Requires mutex lock
	 */
	std::optional<nano::election_actions::vote_broadcast> decide_vote (std::chrono::steady_clock::time_point now);
	...
	nano::uint128_t quorum_delta () const;
