...
bool frontier_round::done_empty () const
{
...
std::optional<nano::account> frontier_round::settle () const
{
	if (!candidates.empty ())
	{
...
void frontier_scan_engine::settle (peer_probes const & probe, std::chrono::steady_clock::time_point now)
{
	for (auto & head : heads)
	{
		if (!head.round)
		...
		if (!ripe)
		{
			...
			if ((pacing_open || capped) && round.inflight () == 0)
			{
				if (capped)
				{
...
std::shared_ptr<frontier_round> frontier_scan_engine::next_round (peer_probes const & probe, std::chrono::steady_clock::time_point now)
{
	// Empty exclusion probes are identical for idle heads, so perform it once per next_round
	auto const idle_peer_probe = probe.peer_status (std::span<nano::account const>{});
	...
	for (size_t offset = 0; offset < heads.size (); ++offset)
	{
		...
		else
		{
			...
			if (idle_peer_probe == peer_probe_status::available)
			{
				auto round = std::make_shared<frontier_round> (config, head.cursor, head.end);
				head.round = round;
				return round;
			}
		}
...
void frontier_scan_engine::advance ()
{
	robin = (robin + 1) % heads.size ();
}
...
nano::container_info frontier_scan_engine::container_info () const
{
	...
	auto tracked_ids = std::accumulate (heads.begin (), heads.end (), std::size_t{ 0 }, [] (auto total, auto const & head) {
		return total + (head.round ? head.round->inflight () : 0);
	});
...
void frontier_scan_engine::conclude (head_state & head, std::chrono::steady_clock::time_point now)
{
	...
	if (target)
	{
		...
		{
			stats.inc (nano::stat::type::bootstrap_frontier_scan, empty_done ? nano::stat::detail::done_empty : nano::stat::detail::done_empty_partial);
		}
