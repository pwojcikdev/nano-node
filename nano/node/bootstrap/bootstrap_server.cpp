...
bool nano::bootstrap_server::verify (const nano::messages::asc_pull_req & message) const
{
	...
	struct verify_visitor
	{
		...
		{
			return !pld.hashes.empty () && pld.hashes.size () <= nano::messages::asc_pull_req::blocks_random_payload::max_hashes;
		}
...
std::deque<std::shared_ptr<nano::block>> nano::bootstrap_server::prepare_topology_blocks (secure::transaction const & transaction, nano::block_hash start_block, std::size_t count) const
{
	...
	for (auto it = std::move (begin); it != end && result.size () < count; ++it)
	{
		...
		if (block)
		{
			...
			auto verify_topo = [&] (auto const & block) {
				...
				for (auto const & dep_hash : block->dependencies ())
				{
					...
					if (dep_block)
					{
						...
						"block " + hash.to_string () + " topo=" + std::to_string (topo) + " dep " + dep_hash.to_string () + " dep_topo=" + std::to_string (dep_block->sideband ().topo_index));
					}
				}
			};
			debug_assert (verify_topo (block); true);
