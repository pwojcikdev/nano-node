...
void bootstrap_context::cleanup_and_sync ()
{
	debug_assert (!mutex.try_lock ());
...
void bootstrap_context::run_timeouts ()
{
...
bool bootstrap_context::process (nano::messages::asc_pull_ack::account_info_payload const & response, async_tag const & tag)
{
	...
	accounts.priority_set (response.account, account_sets_index::priority_cutoff); // Use the lowest possible priority here
