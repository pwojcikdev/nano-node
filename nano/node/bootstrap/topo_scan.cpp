...
void topo_scan_engine::settle (peer_probes const & probe, std::chrono::steady_clock::time_point now)
{
	for (auto & head : heads)
	{
		if (!head.round)
...
std::shared_ptr<topo_round> topo_scan_engine::next_round (peer_probes const & probe, std::chrono::steady_clock::time_point now)
{
	...
	for (size_t offset = 0; offset < heads.size (); ++offset)
	{
		...
		{
			auto round = std::make_shared<topo_round> (config, head.cursor, quorum_l);
			head.round = round;
			return round;
		}
	}
...
bool topo_scan_engine::caught_up () const
{
	bool const pages_inflight = std::any_of (heads.begin (), heads.end (), [] (auto const & head) {
		return head.round != nullptr && head.round->inflight () != 0;
	});
