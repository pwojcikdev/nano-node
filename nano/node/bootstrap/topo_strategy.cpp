...
bool topo_strategy::try_fetch ()
{
	...
	auto cleanup = [&topologies = ctx.topologies] (id_t id, conclusion) {
		topologies.erase_fetch (id, std::chrono::steady_clock::now ());
	};
	return ctx.send (launch->channel, query, strategy::topo, launch->id, std::shared_ptr<round>{}, std::move (cleanup));
}
...
bool topo_strategy::try_page_or_wait ()
{
	...
	return ctx.send (launch.channel, query, strategy::topo, launch.id, launch.round);
}
...
std::optional<topo_strategy::page_wait_result> topo_strategy::next_page_or_ready ()
{
	...
	auto grant = ctx.acquire (strategy::topo, topo_capability (), *round, now);
	ctx.topologies.advance ();
	if (!grant)
	{
		ctx.stats.inc (nano::stat::type::bootstrap_topo_scan, to_stat_detail (grant.peer_status));
		return std::nullopt;
	...
	ctx.stats.inc (nano::stat::type::bootstrap_topo_scan, nano::stat::detail::sample);
	ctx.stats.inc (nano::stat::type::bootstrap_next, nano::stat::detail::topo_index);
	return page_wait_result{ page_wait_result::kind::launch, page_launch{ grant.channel, round, round->position (), grant.id } };
}
