...
void topo_strategy::inspect (nano::secure::transaction const & txn, nano::block_status const & status, nano::block_context const & context)
{
	...
	// Should only inspect blocks originally submitted by this strategy
	debug_assert (std::any_cast<query_source> (context.tag) == query_source::topology_blocks);
	...
	switch (status)
	{
		...
		case nano::block_status::gap_epoch_open_pending:
		{
			...
			ctx.topology.mark_gapped (hash);
			ctx.stats.inc (nano::stat::type::bootstrap_topo, nano::stat::detail::gapped);
		}
		break;
		default:
		{
			// Fork / bad signature / ... — terminally unusable
			ctx.topology.mark_terminal (hash);
			ctx.stats.inc (nano::stat::type::bootstrap_topo, nano::stat::detail::terminal);
		}
		break;
	}
}
...
void topo_strategy::run_one_processing ()
{
	...
	/* last_callback */ {},
	/* tag */ std::any{ query_source::topology_blocks });
...
std::shared_ptr<nano::transport::channel> topo_strategy::wait_topo_channel ()
{
...
bool topo_strategy::request_index (std::size_t h, nano::topo_key cursor, std::shared_ptr<nano::transport::channel> const & channel)
{
	...
	ctx.logger.debug (nano::log::type::bootstrap_topo, "Requesting topo index (head {}) from cursor topo_height={} hash={} from: {}", h, cursor.topo_height, cursor.hash.to_string (), channel);
...
bool topo_strategy::request_blocks (std::deque<nano::block_hash> hashes, std::shared_ptr<nano::transport::channel> const & channel)
{
	...
	ctx.logger.debug (nano::log::type::bootstrap_topo, "Requesting {} random topology blocks from: {}", payload.hashes.size (), channel);
