...
nano::error nano::topo_scan_config::serialize (nano::tomlconfig & toml) const
{
	...
	toml.put ("consideration_count", consideration_count, "Number of distinct peer responses the spear unions at each cursor before advancing the index scan.\ntype:uint64");
	toml.put ("repair_consideration_count", repair_consideration_count, "Number of distinct peer responses a repair head unions per page before advancing. Smaller than consideration_count as repair coverage also builds across wrap-arounds.\ntype:uint64");
...
nano::error nano::bootstrap_config::serialize (nano::tomlconfig & toml) const
{
	...
	toml.put ("enable_topology", enable_topology, "Enable or disable the 'topology` strategy for the ascending bootstrap.\ntype:bool");
	...
	toml.put ("topology_rate_limit", topology_rate_limit, "Rate limit on topology bootstrap requests.\nNote: changing to unlimited (0) is not recommended as this operation competes for resources with realtime traffic.\ntype:uint64");
