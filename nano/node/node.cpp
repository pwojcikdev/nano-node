...
nano::node::node (std::shared_ptr<boost::asio::io_context> io_ctx_a, std::filesystem::path const & application_path_a, nano::node_config const & config_a, nano::work_pool & work_a, nano::node_flags flags_a, unsigned seq) :
	...
	store{ *store_impl },
	wallets_store_impl{ std::make_unique<nano::wallet::mdb_wallets_store> (application_path_a / "wallets.ldb", config_a.lmdb_config) },
	...
	wallets_impl{ std::make_unique<nano::wallet::wallets> (*this, wallets_store, ledger, config, network_params, online_reps, network, stats, logger) },
