...
void nano::network::run_reachout_preconfigured ()
{
	...
	while (!stopped)
	{
		...
		? node.network_params.network.reachout_preconfigured_interval
		: node.network_params.network.reachout_preconfigured_interval * 10; // Less frequent reachout when we have peers
...
void nano::network::trigger_reachout_preconfigured ()
{
	node.stats.inc (nano::stat::type::network, nano::stat::detail::trigger_reachout_preconfigured);
