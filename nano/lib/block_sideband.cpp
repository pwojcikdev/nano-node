...
nano::block_sideband::block_sideband (nano::account const & account_a, nano::block_hash const & successor_a, nano::amount const & balance_a, uint64_t const height_a, nano::seconds_t const timestamp_a, nano::block_details const & details_a, nano::epoch const source_epoch_a, uint64_t const topology_a) :
...
void nano::block_sideband::serialize (nano::stream & stream_a, nano::block_type type_a) const
{
	...
	nano::write (stream_a, boost::endian::native_to_big (topology));
}
...
bool nano::block_sideband::deserialize (nano::stream & stream_a, nano::block_type type_a)
{
	...
	try
	{
		...
		// Migration rewrites all blocks to include this field.
		uint64_t topology_l{ 0 };
		if (!nano::try_read (stream_a, topology_l))
		{
			boost::endian::big_to_native_inplace (topology_l);
		}
		topology = topology_l;
	}
	catch (std::runtime_error &)
