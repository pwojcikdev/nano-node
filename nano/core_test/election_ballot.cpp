...
TEST (vote_info, final)
{
	...
	ASSERT_FALSE (info.final ());
	info.timestamp = final_timestamp;
...
TEST (vote_cooldown, tiers)
{
	...
	ASSERT_EQ (1s, nano::vote_cooldown (6, online_stake)); // More than 5%
	ASSERT_EQ (5s, nano::vote_cooldown (2, online_stake)); // More than 1%
	ASSERT_EQ (15s, nano::vote_cooldown (1, online_stake)); // The rest
}
...
TEST (election_ballot, vote_first_final_bypasses_cooldown)
{
	...
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (rep, final_timestamp, initial->hash (), 10s, epoch + 1s));
...
TEST (election_ballot, vote_final_replacing_final_respects_cooldown)
{
	...
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (rep, final_timestamp, low->hash (), 10s, epoch));
	// A final vote for a lower or equal hash can never advance
	ASSERT_EQ (nano::election_ballot::vote_result::replay, ballot.vote (rep, final_timestamp, low->hash (), 10s, epoch + 20s));
	// A final vote replacing another final vote gets no cooldown bypass
	ASSERT_EQ (nano::election_ballot::vote_result::ignored, ballot.vote (rep, final_timestamp, high->hash (), 10s, epoch + 5s));
	// After the cooldown the switch towards the higher hash passes the tie-break
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (rep, final_timestamp, high->hash (), 10s, epoch + 10s));
}
...
TEST (election_ballot, evaluate_final_quorum)
{
	...
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (reps.rep (7), final_timestamp, initial->hash (), 0s, epoch));
	...
	ASSERT_EQ (7, round1.final_weight);
	...
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (reps.rep (5), final_timestamp, initial->hash (), 0s, epoch));
	auto round2 = ballot.evaluate (10);
	ASSERT_EQ (12, round2.final_weight);
...
TEST (election_ballot, evaluate_final_weight_follows_winner)
{
	...
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (reps.rep (20), nano::vote::timestamp_min, initial->hash (), 0s, epoch));
	ASSERT_EQ (nano::election_ballot::vote_result::accepted, ballot.vote (reps.rep (5), final_timestamp, fork->hash (), 0s, epoch));
