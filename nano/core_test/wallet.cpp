...
TEST (wallet_store, no_special_keys_accounts)
{
	...
	nano::wallet_store wallet{ kdf, env, 1 };
	wallet.create (transaction, nano::dev::genesis_key.pub, "0");
	nano::keypair key1;
...
TEST (wallet_store, serialize_json_empty)
{
	...
	auto transaction (env.tx_begin_write ());
	nano::kdf kdf{ nano::dev::network_params.kdf_work };
	nano::wallet_store wallet1{ kdf, env, 1 };
	wallet1.create (transaction, nano::dev::genesis_key.pub, "0");
	auto serialized = wallet1.serialize_json (transaction);
	nano::wallet_store wallet2{ kdf, env, 1 };
	wallet2.create_from_json (transaction, "1", serialized);
	nano::raw_key password1;
...
TEST (wallet_store, move)
{
	...
	nano::wallet_store wallet2{ kdf, env, 1 };
	wallet2.create (transaction, nano::dev::genesis_key.pub, "1");
	nano::keypair key2;
