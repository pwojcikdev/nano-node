...
TEST (wallet_store, fetch_locked)
{
	...
	ASSERT_TRUE (wallet.valid_password (transaction));
	nano::keypair key1;
	ASSERT_EQ (key1.pub, wallet.insert_adhoc (transaction, key1.prv).value ());
	auto key2 = wallet.deterministic_insert (transaction).value ();
...
TEST (wallet_store, deterministic_keys)
{
	...
	nano::kdf kdf{ nano::dev::network_params.kdf_work };
	nano::wallet::wallet_store wallet (kdf, transaction, backend, nano::dev::genesis_key.pub, 1, "0");
	auto key1 = wallet.deterministic_key (transaction, 0).value ();
	auto key2 = wallet.deterministic_key (transaction, 0).value ();
	ASSERT_EQ (key1, key2);
	auto key3 = wallet.deterministic_key (transaction, 1).value ();
	ASSERT_NE (key1, key3);
	...
	auto key4 = wallet.deterministic_insert (transaction).value ();
	...
	auto key8 = wallet.deterministic_insert (transaction).value ();
	ASSERT_EQ (key4, key8);
	auto key6 = wallet.deterministic_insert (transaction).value ();
	...
	nano::keypair key9;
	ASSERT_EQ (key9.pub, wallet.insert_adhoc (transaction, key9.prv).value ());
...
TEST (wallet_store, reseed)
{
	...
	nano::raw_key seed2;
	seed2 = 2;
	ASSERT_TRUE (wallet.seed_set (transaction, seed1));
	auto seed3 = wallet.seed (transaction).value ();
	ASSERT_EQ (seed1, seed3);
	auto key1 = wallet.deterministic_insert (transaction).value ();
	ASSERT_EQ (1, wallet.deterministic_index_get (transaction));
	ASSERT_TRUE (wallet.seed_set (transaction, seed2));
	ASSERT_EQ (0, wallet.deterministic_index_get (transaction));
	auto seed4 = wallet.seed (transaction).value ();
	ASSERT_EQ (seed2, seed4);
	auto key2 = wallet.deterministic_insert (transaction).value ();
	ASSERT_NE (key1, key2);
	ASSERT_TRUE (wallet.seed_set (transaction, seed1));
	auto seed5 = wallet.seed (transaction).value ();
	ASSERT_EQ (seed1, seed5);
	auto key3 = wallet.deterministic_insert (transaction).value ();
	ASSERT_EQ (key1, key3);
}
