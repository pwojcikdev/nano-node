...
TEST (bootstrap_topo_skip_policy, adapts_threshold_and_decays_history)
{
	nano::topo_scan_config config;
	config.redundant_skip_threshold = 3;
	config.redundant_skip_history_size = 4;
	nano::bootstrap::topo_skip_policy policy{ config };

	// With no need-fetch history, retain the base behavior
	...
	ASSERT_EQ (policy.threshold (), 7);

	// The threshold is frozen for this streak even after redundant pages age all pressure out of the history
	for (auto i = 0; i < 6; ++i)
	{
		ASSERT_FALSE (policy.observe (false));
		ASSERT_EQ (policy.threshold (), 7);
	...
	// The next streak sees the now-clean history and returns to the base threshold
	ASSERT_FALSE (policy.observe (false));
	ASSERT_EQ (policy.threshold (), 3);
	ASSERT_FALSE (policy.observe (false));
	ASSERT_TRUE (policy.observe (false));
