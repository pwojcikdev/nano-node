...
struct test_context
{
	...
	{
		auto round = engine.next_round (probes (), now);
...
TEST (bootstrap_frontier_round, quorum_done)
{
	...
	ASSERT_EQ (round.settle (), nano::account{ 5 });
}

TEST (bootstrap_frontier_round, done_empty)
{
	nano::frontier_scan_config config;
	...
	ASSERT_EQ (round.settle (), nano::account{ 100 });
}
...
TEST (bootstrap_frontier_scan, eager_fanout_tracks_distinct_peers)
{
	...
	ASSERT_EQ (ctx.engine.next_round (ctx.probes (), ctx.now), nullptr);
}
...
TEST (bootstrap_frontier_scan, advance_moves_past_unreserved_round)
{
	nano::frontier_scan_config config;
	config.head_parallelism = 2;
	test_context ctx{ config };
...
TEST (bootstrap_frontier_scan, quorum_settles_clean)
{
	...
	ctx.engine.settle (ctx.probes (), ctx.now);
