...
TEST (bootstrap_server, serve_topo_index_descending)
{
	nano::test::system system{};
	auto & node = *system.add_node ();
	...
	// Start a descending scan from above the top so it returns the highest entries first.
	nano::messages::asc_pull_req request{ node.network_params.network };
	request.id = 9;
	request.type = nano::messages::asc_pull_type::topo_index;

	nano::messages::asc_pull_req::topo_index_payload payload{};
	payload.start = nano::topo_key{ std::numeric_limits<uint64_t>::max (), nano::block_hash{ 0 } };
	payload.count = 1000;
	payload.direction = nano::messages::topo_direction::descending;
	request.payload = payload;
	request.update_header ();
	...
	// Continue descending from the last (lowest) returned entry.
	auto const last = response_payload.entries.back ();
	...
	payload2.start = last;
	payload2.count = 1000;
	payload2.direction = nano::messages::topo_direction::descending;
	request2.payload = payload2;
	request2.update_header ();
	...
	auto response2_payload = std::get<nano::messages::asc_pull_ack::topo_index_payload> (responses.get ().back ().payload);

	// Everything in the continuation is strictly below `last`.
	for (auto const & entry : response2_payload.entries)
	{
		ASSERT_LT (entry, last);
	}
}
