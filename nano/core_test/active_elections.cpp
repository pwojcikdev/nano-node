 ...
 * Blocks still flow through the block processor into the ledger (block pulling),
 * but no election is ever created and nothing is confirmed.
 */
TEST (active_elections, disable_elections)
{
	nano::test::system system;
	nano::node_flags flags;
	...
	// Genesis representative present: with elections enabled this block would normally start an election and confirm
	system.wallet (0)->insert_adhoc (nano::dev::genesis_key.prv);
