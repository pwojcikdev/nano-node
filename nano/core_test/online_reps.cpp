...
/*
 * A vote is observed once, however many of its hashes have an election, and only if at least one has.
 * The representative is observed before the elections get the vote, so their quorum check already sees it online.
 */
TEST (online_reps, vote_observed_once)
{
	...
	// Three elections on independent blocks
	...
	// A vote that no election claims says nothing about its representative
	...
	node.vote_router.vote_matched.add ([&] (std::shared_ptr<nano::vote> const &) {
		// Registered after the node's own observer, so by now the representative counts
