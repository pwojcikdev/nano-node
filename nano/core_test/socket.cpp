...
TEST (socket_functions, count_subnetwork_connections)
{
	...
	auto connection0 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
	auto connection1 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
	auto connection2 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
	auto connection3 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
	auto connection4 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
	auto connection5 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
	auto connection6 = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
...
TEST (socket, disconnection_of_silent_connections)
{
	...
	node->transport.tcp_listener.connection_accepted.add ([&server_data_socket_promise] (auto const & socket, auto const & server) {
		server_data_socket_promise.set_value (socket);
	});

	boost::asio::ip::tcp::endpoint dst_endpoint{ boost::asio::ip::address_v6::loopback (), node->transport.tcp_listener.endpoint ().port () };
...
TEST (socket, DISABLED_drop_policy)
{
	...
	auto func = [&] (size_t total_message_count) {
		...
		auto client = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
...
TEST (socket, DISABLED_concurrent_writes)
{
	...
	accept_callback_t accept_callback = [&] (boost::system::error_code const & ec, boost::asio::ip::tcp::socket socket) {
		if (!ec)
		{
			auto new_connection = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx, std::move (socket));
...
TEST (socket_timeout, connect)
{
	...
	auto socket = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx);
...
TEST (socket_timeout, write)
{
	...
	auto socket = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx, nano::transport::socket_endpoint::client); // socket with a max queue size much larger than OS buffers
...
TEST (socket_timeout, write_overlapped)
{
	...
	// eventually, the all tcp queues should fill up and async_write will not be able to progress
	// and the timeout should kick in and close the socket, which will cause the async_write to return an error
	auto socket = std::make_shared<nano::transport::tcp_socket> (node->transport.ctx, nano::transport::socket_endpoint::client);
